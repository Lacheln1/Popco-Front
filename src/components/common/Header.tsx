...
const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header
      className={`fixed left-0 right-0 top-0 z-50 md:bg-transparent ${
        isScrolled ? "transition-all duration-500 ease-out" : ""
      }`}
    >
      <div
        className={`gmarket ${
          ...
          isScrolled
            ? "mx-auto mt-5 max-w-[1200px] rounded-[60px] bg-white/70 px-6 sm:px-8 md:w-[760px] lg:w-[1200px] lg:px-10"
            : "w-full bg-white/80 text-lg md:bg-white/50"
        }`}
        style={
          isScrolled
            ? {
        ...
        >
          <div
            className={`flex h-[4.5rem] items-center justify-between ${
              isScrolled
                ? "px-0 transition-all duration-500 ease-out"
                : "md:px-8 lg:px-0"
            }`}
          >
            {/* 로고 영역 */}
              ...
              <Link to="/" className="flex items-center">
                <img
                  ...
                  className={`w-auto object-contain ${
                    isScrolled
                      ? "h-12 transition-all duration-500 ease-out"
                      : "h-14"
                  }`}
                />
              </Link>
          ...
          >
            <div
              className={`border-t border-gray-200 pb-4 pt-4 ${
                isScrolled
                  ? "px-6 transition-all duration-500 ease-out sm:px-8 lg:px-10"
              ...
              <div className="flex flex-col space-y-2">
                {navItems.map((item, index) => (
                  <Link
                    key={item.name}
                    to={item.path}
                    className={`h-10 rounded-lg px-4 pt-2 font-medium text-black hover:bg-gray-100 hover:!text-black ${
                      isActiveItem(item.path)
                ...
                {!user.isLoggedIn ? (
                  <button
                    className="h-10 rounded-lg px-4 py-2 text-left font-medium text-black hover:bg-gray-100"
                    onClick={() => {
                  ...
                  <>
                    <button
                      className="rounded-lg px-4 py-2 text-left font-medium text-black hover:bg-gray-100"
                      onClick={() => {
                        setIsMobileMenuOpen(false);
                        handleMyActivity();
                      }}
                    ...
                    <button
                      ...
                      style={
                        isScrolled
                          ? {
                              transitionDelay: `${(navItems.length + 1) * 30}ms`,
                            }
                          : undefined
                      }
                    >
